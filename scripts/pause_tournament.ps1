...
# round robin) or run_custom_trainer.ps1 -- and any Game.exe instances they
# launched, regardless of shard count or format.
#
...
# so one script can pause either of the two run_*.ps1 launchers.)
#
...
# "run_tournament.ps1" (or the other name) matches its own process's
...
$watchdogs = Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" |
    Where-Object { $_.CommandLine -match 'run_tournament\.ps1|run_custom_trainer\.ps1' }
