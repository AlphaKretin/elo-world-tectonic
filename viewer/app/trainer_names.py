...
rules. Loads trainer_card_data.json once and caches the label->row index,
same shape results_lib.load_card_data() returns.
"""
import sys
...
class TrainerNameResolver:
    ...
    falling back to the raw label if trainer_card_data.json hasn't been
    dumped yet or doesn't contain this label (e.g. a hand-typed label in
    Generate that doesn't match any known trainer).

    Every label's display name is computed once, up front, and cached --
    resolve_display_name's per-label cost is O(all card rows) (it re-walks
    every sibling version to work out fight numbering), so calling it fresh
    per Browse-tab row made loading a large results table visibly slow
    (O(rows x card rows)). There are only a few hundred trainer_card_data.json
    rows regardless of how many battle results reference them, so computing
    the whole label -> name mapping once is cheap and every later lookup is
    a plain dict get."""
    ...
    def _ensure_loaded(self):
        ...
        try:
            ...
            self._display_names = {
                label: naming.resolve_display_name(label, card_data_by_label)
                for label in card_data_by_label
            }
