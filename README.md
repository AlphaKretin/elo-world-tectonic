...
Cloud-distributed full singles+doubles round robin is the current plan, using the fleet tooling above — local 8-shard runs so far have been sampled prototypes, not the full round robin. The top-16 bracket (above) hasn't been run against full-RR data either — drafted and validated against the sampled prototype ratings, since it only needs a ratings file to seed from.
